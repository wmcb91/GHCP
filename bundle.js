...
/***/ function(module, exports, __webpack_require__) {
	...
	var profileEvents = __webpack_require__(12);
	var roundsEvents = __webpack_require__(14);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var profileUI = __webpack_require__(9);
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).done(ui.signInSuccess, profileUI.populateProfiles).fail(ui.signInFailure);
	};

	var onChangePasswordClick = function onChangePasswordClick() {
	...
	var onSignOut = function onSignOut(event) {
	  event.preventDefault();
	  api.signOut().done(ui.signOutSuccess, profileUI.clearProfiles).fail(ui.failure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var app = __webpack_require__(7);
	var roundsUI = __webpack_require__(10);
	var handicap = __webpack_require__(11);

	var profilesToObject = function profilesToObject() {
	  var profilesObject = app.user.profiles.reduce(function (object, values, index) {
	    object[index] = values;
	    return object;
	  }, {});
	...
	var populateProfiles = function populateProfiles() {
	  ...
	  for (var i = 0; i < app.user.profiles.length; i++) {
	    ...
	    $('#' + i).show();
	    $('#' + i + i).html("<h3 class='profile-text'>" + profilesObject[i].given_name + "</h3><h4 class='profile-text'>" + profilesObject[i].surname + "</h4><h6 class='profile-club'>" + profilesObject[i].home_course + "</h6></button>");
	    $('#' + i + i).show();
	  }
	};

	var clearProfiles = function clearProfiles() {
	  $('.profile-button').html('');
	  $('.profile-button').hide();
	};
	...
	var showCreateProfile = function showCreateProfile() {
	  ...
	  $('.create-profile').show();
	  setTimeout(function () {
	    $('.create-profile-modal').fadeIn(300);
	  }, 10);
	};

	var showChooseProfile = function showChooseProfile() {
	  $('.create-profile-modal').hide();
	  $('.choose-profile-modal').show();
	  $('#chooseProfileModal').modal('show');
	};

	var backClickSuccess = function backClickSuccess() {
	  $('.create-profile-modal').hide();
	  setTimeout(function () {
	    $('.choose-profile-modal').fadeIn(300);
	  }, 10);
	};
	...
	var showConfirmDelete = function showConfirmDelete() {
	  $('#confirmDeleteModal').modal('show');
	};

	var hideConfirmDelete = function hideConfirmDelete() {
	  $('#confirmDeleteModal').modal('hide');
	};

	var deleteProfileSuccess = function deleteProfileSuccess() {
	  $('#confirmDeleteModal').modal('hide');
	...
	var updateProfileSuccess = function updateProfileSuccess() {
	  $('#user-name-welcome').html(app.profile.given_name);
	  populateProfiles();
	  handicap.updateProfileStats();
	  setTimeout(function () {
	...
	module.exports = {
	  ...
	  selectProfileSuccess: selectProfileSuccess,
	  showChangeProfile: showChangeProfile,
	  showChooseProfile: showChooseProfile,
	  showCreateProfile: showCreateProfile,
	  backClickSuccess: backClickSuccess,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var hideAddRoundField = function hideAddRoundField() {
	  $('.cancel').hide();
	...
	var roundsToObject = function roundsToObject() {
	  var roundsObject = app.profile.rounds.reduce(function (o, v, i) {
	    o[i] = v;
	    return o;
	  }, {});
	  return roundsObject;
	...
	var clearRounds = function clearRounds() {
	...
	module.exports = {
	  ...
	  hideAddRoundField: hideAddRoundField,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var calculateDifferential = function calculateDifferential(round) {
	  var score = round.score;
	  var slope = round.slope;
	  var rating = round.rating;
	  var differential = (score - rating) * 113 / slope;
	  return differential;
	};

	var calcDifferentialsToUse = function calcDifferentialsToUse() {
	  var roundsPlayed = app.profile.rounds.length;
	  var differentialsUsed = void 0;
	  if (roundsPlayed < 5) {
	    differentialsUsed = 0;
	    // Add error message
	...
	var createDifferentialArray = function createDifferentialArray() {
	...
	var createLowestDifferentialArray = function createLowestDifferentialArray(array, differentialsUsed) {
	  var max = differentialsUsed;
	  var differentialArray = array;
	  var lowestDiffArray = [];
	  for (var i = 0; i < max; i++) {
	    var lowestDiff = Math.min.apply(null, differentialArray);
	    var lowDiffIndex = differentialArray.indexOf(lowestDiff);
	...
	var averageDifferential = function averageDifferential(array) {
	  var lowestDiffArray = array;
	  var sum = lowestDiffArray.reduce(function (a, b) {
	    return a + b;
	  }, 0);
	  var averageDifferential = sum / lowestDiffArray.length;
	  return averageDifferential;
	};

	var adjustedAverageDifferential = function adjustedAverageDifferential(avgDifferential) {
	  var avgDiff = avgDifferential;
	  var adjustedAvg = avgDiff * 0.96;
	  return adjustedAvg;
	};

	var truncateAverage = function truncateAverage(avg) {
	  var average = avg;
	  var truncatedAverage = void 0;
	  if (average >= 0) {
	...
	var calculateHandicapIndex = function calculateHandicapIndex() {
	  var differentialsUsed = calcDifferentialsToUse();
	  var differentialArray = createDifferentialArray();
	  var lowestDiffArray = createLowestDifferentialArray(differentialArray, differentialsUsed);
	  var avgDifferential = averageDifferential(lowestDiffArray);
	  var adjustedAvg = adjustedAverageDifferential(avgDifferential);
	  var handicapIndex = truncateAverage(adjustedAvg);
	  app.profile.handicapIndex = handicapIndex;
	  // $('.handicap-index').html(app.profile.handicapIndex);
	};

	var calculateHomeHandicap = function calculateHomeHandicap() {
	  var handicapIndex = app.profile.handicapIndex;
	  var homeSlope = app.profile.home_course_slope;
	  var homeCourseHandicap = Math.round(handicapIndex * (homeSlope / 113));
	  app.profile.homeCourseHandicap = homeCourseHandicap;
	};

	var calculateLowestScore = function calculateLowestScore() {
	  var scoresArray = [];
	  var max = app.profile.rounds.length;
	  for (var i = 0; i < max; i++) {
	    scoresArray.push(parseFloat(app.profile.rounds[i].score));
	  }
	  var lowestScore = Math.min.apply(null, scoresArray);
	  app.profile.lowScore = lowestScore;
	};

	var updateProfileStats = function updateProfileStats() {
	  ...
	  if (app.profile.home_course_slope !== null) {
	    $('.home-handicap').html("<strong>" + app.profile.homeCourseHandicap + "</strong>");
	  } else {
	...
	module.exports = {
	  calculateDifferential: calculateDifferential,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onProfileSelection = function onProfileSelection(event) {
	  event.preventDefault();
	...
	var onCreateProfile = function onCreateProfile(event) {
	  ...
	  api.createProfile(data).done(ui.createProfileSuccess).fail(ui.createProfileFailure);
	  authAPI.getUser().done(authUI.updateUserProfiles, ui.populateProfiles);
	  // Add .fail later
	};

	var onChangeProfileButtonClick = function onChangeProfileButtonClick() {
	  authAPI.getUser().done(authUI.updateUserProfiles, ui.populateProfiles);
	  ui.populateProfiles();
	...
	var onClickDeleteProfile = function onClickDeleteProfile(event) {
	  event.preventDefault();
	  ui.showConfirmDelete();
	};

	var onDeleteProfileReject = function onDeleteProfileReject(event) {
	  event.preventDefault();
	  ui.hideConfirmDelete();
	};

	var onDeleteProfileConfirm = function onDeleteProfileConfirm(event) {
	  event.preventDefault();
	  api.destroyProfile().done(ui.deleteProfileSuccess).fail(ui.deleteProfileFailure);
	  setTimeout(function () {
	    $(authAPI.getUser().done(authUI.updateUserProfiles, ui.populateProfiles));
	  }, 75);
	  setTimeout(function () {
	...
	var onEditProfileButtonClick = function onEditProfileButtonClick() {
	  ui.showEditProfile();
	};

	var onUpdateProfileSubmit = function onUpdateProfileSubmit(event) {
	  event.preventDefault();
	  var data = app.profile;
	  ...
	  api.updateProfile(data).done(ui.updateProfileSuccess).fail(ui.updateProfileFailure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var showProfile = function showProfile(id) {
	  return $.ajax({
	...
	var destroyProfile = function destroyProfile() {
	  return $.ajax({
	    url: app.host + '/profiles/' + app.profile.id,
	    method: 'DELETE',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var profileAPI = __webpack_require__(13);
	...
	var onCancelAddRoundClick = function onCancelAddRoundClick() {
	  ui.hideAddRoundField();
	};

	var getProfileRounds = function getProfileRounds() {
	  profileAPI.showProfile(app.profile.id).done(ui.createRoundSuccess, handicap.calculateHandicap).fail(ui.createRoundFailure);
	};

	//PRESS SUBMIT BUTTON AFTER ENTERING ROUND DATA
	var onSubmitRound = function onSubmitRound(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  data.differential = handicap.calculateDifferential(data);
	  data.profile_id = app.profile.id;
	  api.createRound(data).done(getProfileRounds).fail(ui.createRoundFailure);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var indexRounds = function indexRounds() {
	  return $.ajax({
	    url: app.host + '/rounds',
	    method: 'GET',
	    headers: {
	...
	var createRound = function createRound(data) {
	  return $.ajax({
	    url: app.host + '/rounds',
	    method: 'POST',
	    headers: {
	...
	module.exports = {
	  ...
	  indexRounds: indexRounds,
	  destroyRound: destroyRound
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "beacf9580051e351ee900545827b27a5.jpg";

/***/ },
/* 25 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "2eed7fa81cf11263bb15d74a9b55691f.jpg";

/***/ },
/* 26 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "1ecf19c82fcf1bb438ba060c8ac42630.jpg";

/***/ },
/* 27 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "e1a91b018cd7cc7b520c86a71e45f9c0.jpg";
